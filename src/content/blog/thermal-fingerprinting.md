...
$$Q_{\text{loss}} = UA \cdot \Delta T \qquad \text{(Eq. 1)}$$

Here, U is the thermal transmittance of the envelope assembly (W/m^2K), A is the total envelope area through which heat is lost (m^2), and DeltaT is the temperature difference between the indoor conditioned space and the outdoor air (K or degrees C — the units are equivalent for a difference). The product UA is the overall thermal conductance of the envelope, expressed in watts per kelvin (W/K). A building with poor insulation, single-pane windows, or high air leakage will have a large UA product. A well-insulated, tight building will have a small one. This is the physical quantity we are trying to estimate indirectly.
...
$$Q_{\text{gas}} = \frac{Q_{\text{loss}}}{\eta} = \frac{UA}{\eta} \cdot \Delta T \qquad \text{(Eq. 2)}$$

For this screening methodology, eta is treated as a portfolio constant. The community housing stock under analysis consists predominantly of buildings with mid-efficiency furnaces and boilers installed between the late 1980s and early 2000s. An assumed eta of 0.80 is consistent with the AFUE ratings and age of this equipment. This is a simplification — individual buildings may have had equipment replacements — and Section 8 addresses the implications of this assumption directly. The critical point is that eta is a multiplicative constant applied to the entire portfolio. If the true efficiency varies within a narrow band, the relative ranking of buildings by thermal performance is minimally affected even if the absolute magnitude of estimated UA shifts.

To move from instantaneous heat loss to seasonal gas consumption, the temperature difference DeltaT must be integrated over the heating season. This is precisely what heating degree days represent. The HDD total for a given period is the cumulative sum of the difference between a base temperature (representing the indoor setpoint minus internal heat gains) and the mean daily outdoor temperature, counted only on days when the outdoor temperature falls below the base:

$$\text{HDD} = \sum_{\text{days}} (T_{\text{base}} - T_{\text{outdoor}}) \quad \text{for all days where } T_{\text{outdoor}} < T_{\text{base}} \qquad \text{(Eq. 3)}$$

The Canadian residential standard uses a base temperature of 18 degrees C, reflecting the assumption that a building with a 21 degrees C thermostat setpoint receives approximately 3 degrees C of "free heat" from occupants, lighting, appliances, and solar gains. Using Environment and Climate Change Canada's published monthly HDD values at the 18 degrees C base, the seasonal gas consumption for a building becomes:

$$\text{Gas}_{\text{seasonal}} = \frac{UA}{\eta} \cdot \text{HDD} + b \qquad \text{(Eq. 4)}$$
...
$$\text{Gas} = m \cdot \text{HDD} + b \qquad \text{(Eq. 5)}$$
...
$$m = \frac{UA}{\eta} \qquad \text{(Eq. 6)}$$

This is the result that makes the entire methodology work. The regression slope m — an empirically estimated quantity derived from billing data and weather records — is physically interpretable as the building's overall envelope conductance divided by its mechanical efficiency. A steep slope means the building consumes a large amount of gas for each additional degree day of heating demand. A shallow slope means it does not. The slope encodes the thermal fingerprint of the building.
...
$$UA = U_{\text{avg}} \cdot A_{\text{envelope}} \qquad \text{(Eq. 7)}$$

and substituting into Equation 6:

$$m = \frac{U_{\text{avg}} \cdot A_{\text{envelope}}}{\eta} \qquad \text{(Eq. 8)}$$

The slope m now depends on three quantities: the average envelope U-value (the performance metric we care about), the total envelope area (a geometric property of the building), and the mechanical efficiency (treated as constant). A 200 m^2 townhouse and a 400 m^2 townhouse of identical construction quality will have slopes differing by a factor of two, purely due to size. Ranking by raw slope would penalize the larger building despite identical thermal performance.

The correction is to normalize by a quantity proportional to building size. Conditioned floor area, A_floor, is available from property assessment records and serves as a practical proxy. Dividing the slope by floor area:

$$\frac{m}{A_{\text{floor}}} = \frac{U_{\text{avg}} \cdot A_{\text{envelope}}}{\eta \cdot A_{\text{floor}}} \qquad \text{(Eq. 9)}$$

The ratio A_envelope / A_floor is a geometric constant that depends on building shape — a tall, narrow building has more envelope area per unit of floor area than a squat, wide one. Defining this ratio as the shape factor k:

$$k = \frac{A_{\text{envelope}}}{A_{\text{floor}}} \qquad \text{(Eq. 10)}$$

the normalized slope simplifies to:

$$\frac{m}{A_{\text{floor}}} = \frac{U_{\text{avg}} \cdot k}{\eta} \qquad \text{(Eq. 11)}$$

Since eta is treated as a portfolio constant and k is approximately constant within a building archetype (buildings of the same type and geometry have similar shape factors), the normalized slope m / A_floor becomes proportional to U_avg within each archetype group. This is the metric that enables fair comparison: it isolates the per-unit-area thermal performance of the envelope from the confounding effect of building size.
...
$$A_{\text{footprint}} = \frac{A_{\text{floor}}}{n_{\text{storeys}}} \qquad \text{(Eq. 12)}$$

Assuming a square footprint, the perimeter is:

$$P_{\text{estimated}} = 4 \sqrt{A_{\text{footprint}}} \qquad \text{(Eq. 13)}$$

The exposed wall area depends on the number of exposed faces relative to the total of four possible faces, and on the ceiling height (assumed at 2.7 m as a portfolio constant):

$$A_{\text{walls,exposed}} = P \cdot h_{\text{ceiling}} \cdot n_{\text{storeys}} \cdot \frac{f_{\text{exposed}}}{4} \qquad \text{(Eq. 14)}$$
...
$$A_{\text{envelope,est}} = A_{\text{walls,exposed}} + A_{\text{footprint}} \qquad \text{(Eq. 15)}$$

The 2.7 m ceiling height is an explicit assumption, stated here rather than buried in a calculation. For the housing stock under analysis — predominantly 1970s through 1990s construction — this is a reasonable portfolio constant. Buildings with non-standard ceiling heights (finished basements with lower clearances, for example) introduce modest error that is unlikely to materially affect ranking position.
...
## 5. The Angle Interpretation

The normalized thermal slope m / A_floor is a physically meaningful quantity, but its units — cubic metres of gas per heating degree day per square metre of floor area, or equivalently m^3/(HDD * m^2) — are not immediately intuitive to non-technical audiences. For presentations to municipal councils, housing boards, or senior management, a geometric restatement can improve communication without altering the underlying ranking.

Define the thermal angle theta as:

$$\theta = \arctan\!\left(\frac{m}{A_{\text{floor}}}\right) \qquad \text{(Eq. 16)}$$

This is the angle that the building's normalized regression line makes with the horizontal axis in a plot of gas intensity (gas per unit floor area) versus HDD. A building with a steep normalized slope — high gas consumption per degree day per square metre — produces a large angle. A building with a shallow normalized slope produces a small angle. The visual metaphor is direct: steeper means worse thermal performance.

A clarification on the trigonometric choice is warranted. The arctan function is used here because it computes the angle a line of known slope makes with the x-axis, which is the geometric relationship being described. The arccos and arcsin functions would require different input quantities (a ratio of sides in a right triangle, for instance) and do not apply to this construction.

The critical mathematical property of arctan for this application is that it is monotonically increasing over the domain of positive real numbers. Because every normalized slope in a portfolio of heated buildings is positive (gas consumption increases with HDD), the arctan transformation preserves the rank order exactly. The building with the highest normalized slope will have the highest angle, the building with the second-highest slope will have the second-highest angle, and so on through the entire ranked list. No ties are introduced and no ordering reversals occur.

This means theta carries no information beyond what the normalized slope already provides — it is a communication device, not new analysis. For technical audiences comfortable with regression coefficients, the normalized slope is the more direct metric. For audiences that respond better to visual and geometric language, the angle framing can be substituted without any loss of ranking fidelity. In practice, I have found that presenting a bar chart of "thermal angles" ranked from steepest to shallowest communicates the screening results more effectively in capital planning meetings than a chart of normalized regression coefficients, even though the two charts contain identical information.
...
$$\text{Gas}_{\text{monthly}} = m \cdot \text{HDD}_{\text{monthly}} + b \qquad \text{(Eq. 17)}$$

OLS is appropriate here because the relationship between gas consumption and HDD is expected to be linear over the heating season (following directly from Equation 4), the residuals are expected to be approximately normally distributed, and the method is computationally trivial to implement. More sophisticated regression techniques — robust regression, ridge regression, mixed-effects models — could be applied but add complexity without materially changing the screening results for well-behaved buildings.

Each building's regression should pass a set of quality checks before its slope is used in the portfolio ranking. An R-squared value above 0.85 is expected for buildings where gas consumption is dominated by space heating, the envelope condition is stable, and occupancy is consistent. R-squared values below 0.85 warrant investigation — common causes include substantial non-heating gas loads (a gas-fired pool heater, for example), meter scope issues (a meter serving multiple buildings or a building served by multiple meters), occupancy disruptions, or a mid-period equipment change that shifted the building's thermal signature.
...
Negative intercepts should be flagged as physically implausible. The intercept represents temperature-independent gas consumption — DHW, cooking, pilot lights — and should be a small positive number. A negative intercept suggests a data alignment issue (billing period dates not matching the HDD calculation period), a metering error, or a building that uses an alternative fuel for non-heating loads. Buildings with negative intercepts should be excluded from the ranking until the data issue is resolved.

Conversely, unusually high intercepts are worth flagging as a secondary audit priority. A high intercept indicates that the building has substantial gas consumption unrelated to outdoor temperature, which may signal an oversized or inefficient DHW system, continuous gas appliance loads, or gas consumption being metered for spaces or equipment outside the building envelope. These buildings may offer savings opportunities in non-envelope categories even if their thermal slope is unremarkable.

---

## 7. Synthetic Portfolio Analysis — 100 Buildings

To demonstrate the methodology end-to-end, I constructed a synthetic dataset representing a 100-building community housing portfolio in southern Ontario. The synthetic data is designed to mimic the statistical properties of a real municipal portfolio while allowing controlled variation in the parameters of interest.

The dataset consists of 100 buildings distributed across four archetypes (TH-2S, TH-2S-END, SD, and DT), each with 36 months of simulated gas billing data — three full calendar years. Monthly HDD values are drawn from the historical range for the Kitchener-Waterloo region, approximately 3,400 to 4,000 HDD per year at the 18 degrees C base, with realistic month-to-month variation reflecting the continental climate pattern of cold winters and warm summers. True normalized slopes for each building are drawn from a uniform distribution spanning a realistic performance range, from well-insulated buildings with low gas intensity to poorly insulated buildings with high gas intensity. Random noise is added to each monthly observation to simulate billing estimation variability, meter reading timing misalignment, and weather station representativeness error.

This synthetic dataset stands in for the real municipal portfolio data that will be analyzed in the companion notebook. The purpose here is to demonstrate the mechanics of the methodology — regression fitting, normalization, ranking, and the effect of size-bias correction — in a context where the true underlying performance is known by construction.
...
*Figure 3: The normalization effect. Panel (a) plots raw regression slope m against conditioned floor area, revealing a strong positive correlation — larger buildings have steeper slopes simply because they have more envelope area, not because their envelopes perform worse. Panel (b) plots the normalized slope m / A_floor against conditioned floor area. The correlation disappears, confirming that the normalization successfully removes the size bias and isolates the per-unit-area thermal performance signal.*

The synthetic results confirm several properties of the methodology. First, the regression fits are well-behaved: R-squared values exceed 0.85 for over 90% of the buildings, consistent with the expectation that monthly gas consumption in a heating-dominated residential building is strongly correlated with HDD. Second, the raw slope ranking is visibly confounded by building size (Figure 3a), validating the need for normalization. Third, the normalized ranking (Figure 3b) removes this confound and produces a priority list that reflects envelope performance rather than geometry. Fourth, the top quartile of the normalized ranking — the 25 buildings with the steepest normalized slopes — is the natural candidate list for detailed audit. These buildings are not confirmed as the worst performers (regression to the mean is a real concern, discussed in Section 8), but they are the buildings where a detailed audit is most likely to yield actionable findings.

The intercept distribution across the portfolio also provides useful secondary information. Buildings with intercepts more than two standard deviations above the portfolio mean are flagged for investigation of non-heating gas loads. In the synthetic dataset, these outliers correspond to buildings where the simulated baseload was deliberately set high, confirming the flag's discriminating power.
...
The screening methodology proceeds through a defined sequence of steps, from data collection through to capital planning integration. The workflow is designed to be executed by an energy analyst or sustainability coordinator with access to utility billing records, property assessment data, and basic statistical software (Python with pandas and scikit-learn, R, or even a well-structured spreadsheet for small portfolios).

The first step is to classify each building in the portfolio into an archetype using property assessment records, housing authority databases, or a brief site survey. The classification should capture building type (townhouse, semi-detached, detached, apartment block), storey count, and the number of exposed wall faces. For community housing portfolios with standardized construction, this classification can often be completed from records alone, supplemented by satellite imagery for exposure confirmation. The archetype table in Section 4 provides the recommended classification scheme.

The second step is to assign a shape factor k to each building. For archetypes with regular, repeated geometry (Tier 1), a shape factor is computed from representative unit dimensions and applied to all buildings in the class. For archetypes with variable geometry (Tier 2), the shape factor is computed individually using the square footprint approximation described in Equations 12 through 15. The ceiling height assumption of 2.7 m is applied as a portfolio constant.

The third step is to collect a minimum of three years of monthly gas billing data for each building, aligned to monthly HDD values from the nearest representative weather station. Billing periods rarely align perfectly with calendar months, so the analyst should either prorate billing data to calendar months or compute HDD values matching the actual billing period dates. The latter approach is more accurate but requires billing period start and end dates, which are not always readily available.

The fourth step is to run an ordinary least squares regression for each building, with monthly gas consumption as the dependent variable and monthly HDD as the independent variable (Equation 17). This produces a slope m and intercept b for each building in the portfolio.

The fifth step is to apply quality checks to each regression. The R-squared value should exceed 0.85 for well-behaved buildings. Residual plots should be inspected for seasonal patterns. Negative intercepts should be flagged as physically implausible and investigated. Unusually high intercepts should be noted as secondary audit candidates.

The sixth step is to compute the normalized thermal slope for each building. Using the Tier 1 or Tier 2 envelope area estimate, the normalized slope is m / A_envelope_estimated. Alternatively, if using floor area normalization with archetype grouping, the simpler m / A_floor can be used for within-archetype ranking, since the shape factor k is constant within the group and cancels in the comparison.

The seventh step is to rank buildings within each archetype group by normalized thermal slope, from highest (worst thermal performance, highest audit priority) to lowest (best thermal performance, lowest priority).

The eighth step, which is optional, is to produce a cross-portfolio ranking combining all archetype groups. This ranking should carry explicit caveats about inter-archetype geometric uncertainty and should be presented alongside — not in place of — the within-archetype rankings.

The ninth step is to flag buildings with anomalously high intercepts (more than two standard deviations above the archetype mean) as secondary audit priorities. These buildings may have substantial non-envelope savings opportunities in DHW, cooking, or other baseload categories.

The tenth step is to define the audit candidate list. The top quartile within each archetype group — the 25% of buildings with the highest normalized thermal slopes — forms the prioritized list for detailed energy audits. This quartile threshold is a practical default; the actual cutoff should be calibrated to the available audit budget and the number of buildings the housing authority can reasonably investigate in a given planning cycle.

The eleventh and final step connects the screening back to the capital planning cycle. After detailed audits are completed and retrofits are implemented, the regression should be re-run using post-retrofit billing data. A reduction in the regression slope is the measurable signal that the retrofit improved the building's thermal performance. This before-and-after comparison, using the same methodology applied during screening, provides a defensible measurement and verification framework for reporting energy savings to municipal councils and provincial funding agencies.

This workflow connects directly to municipal climate action planning. The ranked priority list provides an evidence base for capital improvement submissions, the normalized slopes provide a quantitative performance metric for GHG reduction target tracking, and the before-and-after regression comparison provides a verification mechanism that satisfies reporting requirements. The methodology transforms gas billing data — a dataset collected for financial purposes — into a planning tool for climate and asset management decisions.

---

## 10. Conclusion

The thermal fingerprinting methodology described in this post extracts a building-level thermal performance signal from two datasets that every municipal housing authority already collects: monthly gas bills and regional weather records. By grounding the analysis in the steady-state heat loss equation and mapping it to a simple linear regression, the approach produces a physically interpretable metric — the normalized thermal slope — that ranks buildings by envelope-plus-mechanical performance within archetype groups. The result is a prioritized candidate list for detailed energy audits, generated at negligible incremental cost.
...
The methodology is low-cost because it uses existing data. It is repeatable because the regression procedure is fully specified and deterministic. It is transparent because every assumption — the constant mechanical efficiency, the fixed HDD base temperature, the shape factor approximation, the ceiling height — is stated explicitly and can be tested or modified by the analyst applying it. And it is extensible: the same regression framework that produces the screening ranking also provides the before-and-after measurement framework for verifying retrofit performance.
